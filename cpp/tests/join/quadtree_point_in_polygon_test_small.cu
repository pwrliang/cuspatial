...
TYPED_TEST(PIPRefineTestSmall, TestMultipolygon)
{
  using T = TypeParam;
  using cuspatial::vec_2d;
  using cuspatial::test::make_device_vector;
  ...
  auto bboxes =
    rmm::device_uvector<cuspatial::box<T>>(multipolygons.num_multipolygons(), this->stream());

  cuspatial::polygon_bounding_boxes(multipolygons.geometry_offset_begin(),
                                    multipolygons.geometry_offset_end(),
