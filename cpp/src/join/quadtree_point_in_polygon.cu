...
struct compute_quadtree_point_in_polygon {
    ...
    rmm::device_async_resource_ref mr)
  {
    ...
    auto multipolygons =
      multipolygon_range(geom_offsets.begin<uint32_t>(),
                         geom_offsets.end<uint32_t>(),
                         part_offsets.begin<uint32_t>(),
                         part_offsets.end<uint32_t>(),
                                                       ...
                                                       rmm::device_async_resource_ref mr)
{
  ...
  CUSPATIAL_EXPECTS(geom_offsets.type() == part_offsets.type(),
                    "offset columns must have the same data type");
  CUSPATIAL_EXPECTS(part_offsets.type() == ring_offsets.type(),
                    "offset columns must have the same data type");

  if (poly_quad_pairs.num_rows() == 0 || quadtree.num_rows() == 0 || point_indices.size() == 0 ||
      geom_offsets.size() == 0 || part_offsets.size() == 0) {
    std::vector<std::unique_ptr<cudf::column>> cols{};
    cols.reserve(2);
